...
const register = async (req, res) => {
  ...
  try {
    ...
    await db.batch([
      {
        sql: `INSERT INTO users (id, name, email, password, is_verified, verification_code, code_expires_at, trial_ends_at, plan, subscription_status) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'free', 'trial')`,
        args: [userId, name, email, hashedPassword, 0, code, expiresAt, trialEndsAt]
      },
      {
...
//     const normalizedEmail = email;
//     const result = await db.execute({
//       sql: 'SELECT id, name, email, pending_email, verification_code, code_expires_at FROM users WHERE email = ? OR pending_email = ?',
